...
# Global constants for a wall and empty space
RIGHT = 0
DOWN = 1
LEFT = 2
TOP = 3
...
def check(new_maze,row_input,column_input,final_r,final_c,path):
    ...
    print("row is", row_input, "and col is", column_input)
    ...
    if row_input == final_r:
        if column_input == final_c:
            print("Found a solution at(",row_input,",",column_input,")")
            return True
    ...
    if int(new_maze[row_input][column_input][RIGHT]) == SPACE and (row_input,column_input+1) not in path :
        ...
        print("(",row_input,",",column_input,")")
        check(new_maze,row_input,column_input,final_r,final_c,path)
        return True
        if (row_input,column_input) in path and int(new_maze[row_input][column_input][RIGHT]) == SPACE:
            path.remove((row_input,column_input+1))
            wrong_path.append((row_input,column_input))
   ...
   # if int(new_maze[row_input][column_input][RIGHT]) + int(new_maze[row_input][column_input][DOWN]) + \
    #        int(new_maze[row_input][column_input][LEFT]) + int(new_maze[row_input][column_input][TOP]) == 3:
...
def main():
    ...
    # Find better way to inverse previous rather than make another loop
    index = -1
    while copy_path[index] != (final_r,final_c):
        final_path.append(copy_path[index])
        index-=1
    final_path.append((final_r,final_c))
    
    # Loop that prints coordinates that solve the maze
